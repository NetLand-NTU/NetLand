...
public class LinePlot extends ScatterPlot {
	...
	public void plot(AbstractDrawer draw, Color c) {
		...
		//modified plot 100 points
		double[][] tempXY = null;
		
		if( XY.length > 100 ){
			int step = XY.length/100;
			int newNum = (XY.length-1)/step+1;
			tempXY = new double[newNum][2];
			for(int i=0;i<XY.length;i+=step){
				tempXY[(int)(i/step)][0] = XY[i][0];
				tempXY[(int)(i/step)][1] = XY[i][1];
			}
		}else
			tempXY = XY;
...
//			draw.drawLine(XY[i], XY[i + 1]);
		...
		for (int i = 0; i < tempXY.length - 1; i++)
			draw.drawLine(tempXY[i], tempXY[i + 1]);
	}
