...
extends Object
{
	...
	private double doOperation(int beg, double lft, Operator opr, double rgt) {
		...
		try { 
			double output = opr.handler.evaluateOperator(lft,opr.symbol,rgt);
			return output; 
		}
...
//			case '*' : return judgeIfOverflow(lft,rgt,1);                                                              // multiply (classical)
//			case '×' : return judgeIfOverflow(lft,rgt,1);                                                               // multiply (because it's a Unicode world out there)
//			case '·' : return judgeIfOverflow(lft,rgt,1);                                                                 // multiply (because it's a Unicode world out there)
//			case '(' : return judgeIfOverflow(lft,rgt,1);                                                              // multiply (implicit due to brackets, e.g "(a)(b)")
//			case '/' : return judgeIfOverflow(lft,rgt,2);                                                                 // divide (classical computing)
//			case '÷' : return judgeIfOverflow(lft,rgt,2);                                                                  // divide (because it's a Unicode world out there)
//			case '%' : return lft%rgt;                                                              // remainder
//			case '+' : return judgeIfOverflow(lft,rgt,3);                                                                 // add/unary-positive
//			case '-' : return judgeIfOverflow(lft,rgt,4);                                                                 // subtract/unary-negative
...
//		}
		...
		private double judgeIfOverflow(double lft, double rgt, int type){
			//type: 1 * 2 / 3 + 4 -
			if( type == 1 ){
				if( Double.MAX_VALUE/rgt < lft ) return Double.NaN;
				return lft*rgt;
			}else if( type == 2 ){
				if( Double.MIN_VALUE*rgt > lft ) return Double.NaN;
				return lft/rgt;
			}else if( type == 3 ){
				if( Double.MAX_VALUE - lft < rgt ) return Double.NaN;
				return lft+rgt;
			}else if( type == 4 ){
				if( Double.MIN_VALUE+rgt > lft ) return Double.NaN;
				return lft-rgt;
